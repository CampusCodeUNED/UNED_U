...
public final class ValidadorOrdenEstructural {
    ...
    public static final class Result {
        ...
        // Resultado del análisis de orden estructural
        public final List<Diagnostico> diags;
        public final int lineaProgram, lineaUses, lineaConst, lineaVar, lineaBegin;

        public Result(List<Diagnostico> diags, int lineaProgram, int lineaUses,
    ...
    // Método principal para analizar el orden estructural
    public Result analyze(Fuente fuente) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // Quitar comentarios para buscar palabras clave
            String sinComentarios = quitarComentarios(linea).toLowerCase(Locale.ROOT).trim();

            int numeroLinea = i + 1;

            if (lProgram < 0 && sinComentarios.contains("program")) {
                lProgram = numeroLinea;
            }
            if (lUses < 0 && sinComentarios.contains("uses")) {
                lUses = numeroLinea;
            }
            if (lConst < 0 && sinComentarios.contains("const")) {
                lConst = numeroLinea;
            }
            if (lVar < 0 && sinComentarios.matches("\\s*var\\b.*")) {
                lVar = numeroLinea;
            }
            if (lBegin < 0 && (sinComentarios.equals("begin") || sinComentarios.startsWith("begin "))) {
                lBegin = numeroLinea;
            }
        ...
        return new Result(diags, lProgram, lUses, lConst, lVar, lBegin);
    }

    // Método auxiliar para quitar comentarios de una línea
