...
public final class ValidadorComentarios {
    ...
    private void validarComentariosEnLinea(String linea, int numeroLinea, int lineaProgram, int lineaEnd, List<Diagnostico> diags) {
        ...
        // Validar espacios en comentarios
        validarEspaciosEnComentarios(linea, numeroLinea, diags);
    }
    ...
    private boolean pareceIntentoComentario(String linea, int posicionSlash) {
        ...
        // y está al final de la línea o hay otro / después de espacio
        String restoLinea = linea.substring(posicionSlash + 1);
        if (restoLinea.matches("\\s*/.*")) {
            return true; // Es un / seguido de espacio y otro /
        }
        
        return restoLinea.trim().length() > 2 && restoLinea.trim().matches(".*[A-Za-z].*");
    }

    // Validar que no haya espacio después de // o {
    private void validarEspaciosEnComentarios(String linea, int numeroLinea, List<Diagnostico> diags) {
        // Buscar // con espacio después
        if (linea.matches(".*//\\s+.*")) {
            diags.add(new Diagnostico(E_COMENTARIO_SLASH_CON_ESPACIO, numeroLinea, null,
                    "No debe haber espacio después de //"));
        }
        
        // Buscar { con espacio después o antes de }
        if (linea.matches(".*\\{\\s+.*") || linea.matches(".*\\s+\\}.*")) {
            diags.add(new Diagnostico(E_COMENTARIO_SLASH_CON_ESPACIO, numeroLinea, null,
                    "No debe haber espacios después de { o antes de }"));
        }
    }
    ...
    public static boolean esLineaComentario(String linea) {
        ...
        return contenido.startsWith("{") && contenido.endsWith("}");
    }
